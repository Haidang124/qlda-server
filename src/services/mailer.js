...
const { handleErrorResponse, handleSuccessResponse, getCurrentId } = require('../helper/responseHelper');
const config = require("../../config");
const User = require('../model/userModel');
const confirmMail = require("../model/confirmMailModel");
const Project = require('../model/projectModel');
const { joinProject } = require('../app/controllers/projectController');
...
module.exports.sendEmail = async (req, res) => {
  ...
  let project = await Project.findOne({ $or: [{ userId: member._id.toString() }, { userJoin: member._id.toString() }] });
  if (project) {
    return handleErrorResponse(res, 400, "MemberInProject");          // member đã tham gia project
  }
  if (!(await confirmMail.findOne({ projectId: projectId, userInvited: member._id }))) {
    let code = new confirmMail({ projectId: projectId, userInvite: userId, userInvited: member._id });
          ...
          transporter.sendMail(mainOptions, function (err, inf) {
            if (err) {
              return handleErrorResponse(
                res, 400, "ErrorSendEmail"
              );
            } else {
              return handleSuccessResponse(
                res, 200, {}, "SuccessSendEmail"
              );
            }
          });
        }
    ...
    let code = await confirmMail.findOne({ projectId: projectId, userInvited: member._id });
    let mainOptions = {
      from: "qlda.lienhe@gmail.com",
      to: email,
      subject: "Mail mời tham gia dự án",
...
module.exports.checkConfirmEmail = async (req, res) => {  //req: {confirmId, memberId}
  ...
  if (confirm.userInvited.toString() === memberId) {
    ...
    return handleSuccessResponse(res, 200, { projectId: confirm.projectId.toString() }, "SuccessComfirm");
  }
  return handleErrorResponse(res, 400, "FailCheckComfirm");
}
